...
all:
 ...
 test - run all tests\n\
 pack - make output archive, file name format \"fft_vX.Y.Z_BRANCHNAME.tar.gz\"\n\
"
...
pack: make_build
	rm -f fft-*.tar.gz
	echo Create archive \"fft-$(VERSION)-$(BRANCH).tar.gz\"
	cd make_build; tar czf ../fft-$(VERSION)-$(BRANCH).tar.gz fft
...
fft.tar.gz: build
	cd make_build; tar czf ../fft.tar.gz fft && rm -rf ../make_build
